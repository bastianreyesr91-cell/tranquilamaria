...
export function classificationLabel(type, classification) {
    const isFijo = classification === 'FIJO';
    if (type === 'INCOME') return isFijo ? 'Ingreso Fijo' : 'Ingreso Variable';
    return isFijo ? 'Gasto Fijo' : 'Gasto Variable';
}

export function classificationColor(classification) {
    return classification === 'FIJO' ? '#60a5fa' : '#fb923c';
}
